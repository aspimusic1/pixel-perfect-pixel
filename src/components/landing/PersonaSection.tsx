...
import { ArrowRight, Mic2, Users, Building2, Wrench, Camera, Check, X } from "lucide-react";
...
const PERSONAS: Record<PersonaKey, Persona> = {
  artist: {
    ...
    icon: Mic2,
    label: "Artist",
    badge: "built for artists — own your career",
    headline: "Own Your Bookings.\nOwn Your Career.",
    subtext:
      "Stop chasing gigs through DMs. Receive structured offers, negotiate in deal rooms, and get paid on time — all from one dashboard.",
    cta: "Join as Artist",
    ctaLink: "/auth?mode=signup&role=artist",
    painTitle: "Why artists switch to GetBooked.Live",
    pains: [
      "Chasing promoters through Instagram DMs",
      "No idea if an offer is fair for your market",
      "Contracts done on napkins or not at all",
  ...
  promoter: {
    ...
    icon: Users,
    label: "Promoter",
    badge: "built for promoters — book smarter",
    headline: "Book the Right Artist.\nEvery Time.",
    subtext:
      "Find verified talent, send structured offers, and manage every booking from confirmation to settlement.",
    cta: "Join as Promoter",
    ctaLink: "/auth?mode=signup&role=promoter",
    painTitle: "Why promoters switch to GetBooked.Live",
    pains: [
      "Hours spent searching for the right artist",
      "Unstructured offers lost in email threads",
      "No reliable attendance data to vet talent",
      "Manually tracking deposits, riders, and contracts",
    ],
    features: [
  ...
  venue: {
    ...
    icon: Building2,
    label: "Venue",
    badge: "built for venues — fill your calendar",
    headline: "Fill Your Calendar.\nGrow Your Revenue.",
    subtext:
      "List your space, showcase capacity and amenities, and connect directly with promoters and artists looking for rooms.",
    cta: "Join as Venue",
    ctaLink: "/auth?mode=signup&role=venue",
    painTitle: "Why venues switch to GetBooked.Live",
    pains: [
      "Empty weeknights and underbooked seasons",
      "No centralized way to list availability",
      "Promoters ghost after initial inquiry",
      "No data on which artists actually draw",
    ],
    features: [
  ...
  production: {
    ...
    icon: Wrench,
    label: "Production",
    badge: "built for production crews — get hired",
    headline: "Get Booked for\nthe Shows That Matter.",
    subtext:
      "Access real booking opportunities and get hired directly by promoters and venues. No more hustling for gigs through personal networks.",
    cta: "Join as Production",
    ctaLink: "/auth?mode=signup&role=production",
    painTitle: "Why production crews switch to GetBooked.Live",
    pains: [
      "Hard to find consistent, well-paying gigs",
      "Scattered communication with promoters and venues",
      "No centralized job pipeline for live events",
      "Technical requirements unclear until last minute",
    ],
    features: [
  ...
  creative: {
    ...
    icon: Camera,
    label: "Creative",
    badge: "built for creatives — bring shows to life",
    headline: "Bring Shows to\nLife Creatively.",
    subtext:
      "Get hired to create photos, video content, and branding for live events and tours. Your portfolio does the talking.",
    cta: "Join as Creative",
    ctaLink: "/auth?mode=signup&role=photo_video",
    painTitle: "Why creatives switch to GetBooked.Live",
    pains: [
      "Hard to connect with live events that need visuals",
      "Last-minute gig sourcing through Instagram DMs",
      "No centralized creative opportunities for events",
      "Portfolio scattered across different platforms",
    ],
    features: [
...
const ECOSYSTEM_FLOWS: Record<PersonaKey, { label: string; color: string }[]> = {
  promoter: [
    { label: "Promoter", color: "#FF5C8A" },
    { label: "Artist", color: "#C8FF3E" },
    { label: "Venue", color: "#FFB83E" },
    { label: "Production", color: "#7B5CF0" },
    { label: "Creative", color: "#3EC8FF" },
  ],
  artist: [
    { label: "Promoter", color: "#FF5C8A" },
    { label: "Artist", color: "#C8FF3E" },
    { label: "Deal Room", color: "#C8FF3E" },
    { label: "Tour Hub", color: "#C8FF3E" },
  ],
  venue: [
    { label: "Promoter", color: "#FF5C8A" },
    { label: "Venue", color: "#FFB83E" },
  ...
  production: [
    ...
    { label: "Production", color: "#7B5CF0" },
    { label: "Venue", color: "#FFB83E" },
    { label: "Show Night", color: "#7B5CF0" },
  ],
  creative: [
    { label: "Artist", color: "#C8FF3E" },
    { label: "Creative", color: "#3EC8FF" },
    { label: "Content", color: "#3EC8FF" },
    { label: "Fan Reach", color: "#3EC8FF" },
  ],
};
  ...
  return (
    <section className="fade-in-section py-16 sm:py-28 px-4">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="flex justify-center gap-2 mb-8">
          {INTENTS.map((i) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-display font-semibold transition-all duration-200 active:scale-[0.96] lowercase ${
                intent === i.key
          ...
          {PERSONA_KEYS.map((key) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-display font-medium transition-all duration-300 active:scale-[0.96] lowercase max-w-[120px] flex-1 min-w-[100px] ${
                  isActive
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-card/60 p-7 sm:p-10 transition-all duration-300">
          {/* Badge */}
          <span
          ...
          <p className="text-sm text-muted-foreground font-body leading-relaxed max-w-lg mb-8">{persona.subtext}</p>
          ...
          <ul className="space-y-2 mb-8">
            {persona.pains.map((pain, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-foreground/70 font-body">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
            {persona.features.map((f) => (
              <div key={f.name} className="rounded-xl bg-secondary/40 border border-white/[0.06] p-4">
          ...
          <div className="flex flex-wrap gap-4 sm:gap-8 mb-8">
            {persona.stats.map((s) => (
              <p key={s} className="text-xs font-display font-bold text-muted-foreground uppercase tracking-wider">
                {s}
              </p>
            ))}
          </div>
          ...
          <Link to={persona.ctaLink}>
            <button
              className="font-display font-bold text-sm rounded-[10px] px-7 h-11 active:scale-[0.96] transition-all inline-flex items-center gap-2 lowercase"
